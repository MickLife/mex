...
└─────────────┘                        │  ③ 组装抽取 prompt          │
                              ...
                              │ 收到：抽取规范 prompt + 对话文本       │
                              │ 自主判断哪些值得记，调用               │
                              │ mex_search / mex_add / mex_update   │
                              └────────────┬────────────────────────┘
                                           │ ⑥ 调用 mex 工具
                                           ▼
                              ┌─────────────────────────────────────┐
                              │ mex_add 等工具（index.js 注册）       │
                              │ 薄壳：subprocess 执行 mex CLI 命令     │
                              │   mex add --topic work --content ... │
                              └────────────┬────────────────────────┘
                                           │ ⑦ CLI 写数据库
                                           ▼
                              ┌─────────────────────────────────────┐
...
- 创建一个**全新的子代理**：独立会话、独立上下文、**看不到用户与 AI 的主对话**（零父上下文）。
- 把 prompt 作为子代理的第一条用户消息，子代理运行完一轮后返回结果并自动销毁。
...
```js
const result = await run.result          // 等子代理完成
extractedTurns.set(sessionId, lastTurn)  // 标记这轮已抽取，防重复
sendJson(res, 200, {
  ok: true,
  written: parseWritten(result.output),  // 从"已写入N条记忆"解析条数
...
| 接口 | 方法 | 入参 | 返回 |
|---|---|---|---|
| `/mex/panel-state` | GET | `sessionId` query | `{ ok, hasNew, lastTurn, extractedTurn }` |
| `/mex/extract` | POST | body `{ sessionId }` | `{ ok, written, stopReason, error? }` |
...
- 每次点击都启动一个子代理（一次 LLM 调用）；高频点击会放大成本——当前由"同一轮防重复"机制兜底。
